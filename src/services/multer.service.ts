...
// multer upload
export const createMulterUploader = (config: MulterConfig = {}) => {
  // destruc
  const { uploadPaths = {}, allowedMimeTypes = /jpeg|jpg|png|pdf|webp/ } =
    config;
  ...
  return {
    ...
    fields: (fileds: { name: string; limit?: number }[]) =>
      uploader.fields(fileds),
  };
};
