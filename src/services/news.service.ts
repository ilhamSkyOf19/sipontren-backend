...
import { CreateNewsType, ResponseNewsType, toResponseNews, UpdateNewsType } from "../models/news-model";
...
export class NewsService {
    ...
    // create 
    static async create(req: CreateNewsType, thumbnail: string): Promise<ResponseData<ResponseNewsType>> {
    ...
    static async read(): Promise<ResponseData<ResponseNewsType[]>> {
        ...
        const data = response.map((news) => {
            const url_thumbnail = `${process.env.BASE_URL}/news/${news.thumbnail}`
            return {
                ...news,
                url_thumbnail
            }
    ...
    // update 
    static async update(id: number, req: UpdateNewsType, thumbnail: string | undefined): Promise<ResponseData<ResponseNewsType>> {
